export class SmsService {
   ...
   */
  static async sendWarningSms(mobileNumber: string, vehicleNumber: string, violationType: string) {
    const message = `Hello, this is a testing message only. Please drive safely.`;
    return this.sendCustomSms(mobileNumber, message);
  }

  static async sendCustomSms(mobileNumber: string, message: string) {
    if (!mobileNumber) {
